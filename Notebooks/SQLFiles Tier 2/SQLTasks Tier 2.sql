...
in question. */
SELECT name, CASE WHEN monthlymaintenance >= 100 THEN "expensive" ELSE "cheap" END  AS label
FROM Facilities
...
FROM Members
WHERE joindate in (SELECT MAX(joindate) FROM Members) OR joindate in (SELECT MIN(joindate) FROM Members)
...
JOIN (SELECT name, facid, membercost, guestcost
      FROM Facilities)AS f ON f.facid = b.facid
